...
public interface WnwbExternalrefRepository extends JpaRepository<WnwbExternalref, Integer> {
    ...
    @Query("SELECT e FROM WnwbExternalref e WHERE e.reference = :reference AND e.sysId.localLexicon IS NOT NULL AND e.isDeleted = false")
    List<WnwbExternalref> findByReferenceWithLocalLexicon(@Param("reference") String reference);

    @Query("SELECT e FROM WnwbExternalref e WHERE e.reference = :reference AND e.sysId = :system AND e.synset IS NOT NULL AND e.isDeleted = false")
    List<WnwbExternalref> findByReferenceAndSystem(@Param("reference") String reference, @Param("system") WnwbExternalsystem system);
